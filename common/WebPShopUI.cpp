...
void WebPShopDialog::Init(void) {
  ...
  std::string webp_str = "WebP 设置:";
  if (write_config_.animation) {
    webp_str = "动画 " + webp_str;
  }
...
void WebPShopDialog::PaintProxy(void) {
  ...
  proxy_checkbox_.SetText("预览: " + DataSizeToString(encoded_data_->size));
